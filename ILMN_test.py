...
"""
run modelTest to:
    fetch the last month of stock prices.
    shift the data to create a time series input for machine learning.
    call a pretrained GBT model to predict each day's closing price and graph vs actual close
...
def modelTest(name):
    ...
    #add columns t-1,t-2...t-n,vt-1,vt-2...vt-n
    for i in range(1,10):
    ...
    #add new row to predict tomorrow's price
    #set today's closing price to yesterday in order to predict tomorrow
